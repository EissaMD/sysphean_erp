...
class PartNo (DB,Page):
    ...
    def Edit_Filled_frame(self):
        ...
        self.carton_view_sheet = Sheet(carton_view_frame, show_x_scrollbar=False, height=400,
                                   headers=["ID", "Part No", "Carton Quantity", "Date Codes", "Remarks", "Packing Date", "Time Created", "User"])
        ...
        carton_data = DB.select("carton_table", (
            "id", "part_no", "carton_quantity", "date_codes", "remarks", "packing_date", "time_created", "user_name"), "loose_quantity = 0 AND (delivery_id = 0 OR delivery_id IS NULL OR CHAR_LENGTH(delivery_id & '') = 0) ORDER BY id DESC")
    ...
    def carton_filter_track_table(self, part_no):
        ...
        carton_data = DB.select("carton_table", (
            "id", "part_no", "carton_quantity", "date_codes", "remarks", "packing_date", "time_created", "user_name"), "part_no LIKE %s AND loose_quantity = 0 AND (delivery_id = 0 OR delivery_id IS NULL OR CHAR_LENGTH(delivery_id & '') = 0) ORDER BY id DESC",
                              ("%%" + part_no + "%%",))
        for row_data in carton_data:
            self.carton_view_sheet.insert_row(values=row_data)
    ###############        ###############        ###############        ###############
    ...
    def Edit_Sealed_frame(self):
        ...
        sealed_data = DB.select("sealed_inventory", (
            "id", "part_no", "quantity", "date_code", "remarks", "additional_info", "time_created", "user_name"), "1=1 ORDER BY id DESC")
    ...
    def sealed_filter_track_table(self, part_no):
        ...
        sealed_data = DB.select("sealed_inventory", (
            "id", "part_no", "quantity", "date_code", "remarks", "additional_info", "time_created", "user_name"), "part_no LIKE %s ORDER BY id DESC",
                              ("%%" + part_no + "%%",))
        for row_data in sealed_data:
            self.sealed_view_sheet.insert_row(values=row_data)
    ###############        ###############        ###############        ###############
