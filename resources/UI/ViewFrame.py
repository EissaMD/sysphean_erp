...
class ViewFrame(ctk.CTkFrame):
    def __init__(self,master , select_ls=["First" , "Secound" , "Third"]):
        ...
        self.view_body = ctk.CTkFrame(self,fg_color="transparent", border_width=2)
        self.view_body.pack(fill="both",expand=True ,pady=5,padx=4)
        self.select_item(select_ls[0])
    ###############        ###############        ###############        ###############
    def select_item(self,choice):
        self.view_body.destroy()
        self.view_body = ctk.CTkFrame(self,fg_color="transparent", border_width=2)
        self.view_body.pack(fill="both",expand=True ,pady=5,padx=4)
        SearchFrame(self.view_body,choice).pack(fill="both" , expand=True)
##############################################################################################################
