...
class SimilarBatchWindow(ctk.CTkToplevel):
    def __init__(self,data):
        ...
        sealed_records = DB.select("entry_tracker",columns,"part_no = %s AND date_code=%s",tuple(data.values()))
        if len(sealed_records) == 0:
            return
        super().__init__()
        self.title("Similar Batch")
        self.geometry("1000x310")
        # Title
        ...
        ctk.CTkButton(self, text="Continue ?",command=self.continue_btn,width=150).pack(pady=2,)
        self.deiconify()
